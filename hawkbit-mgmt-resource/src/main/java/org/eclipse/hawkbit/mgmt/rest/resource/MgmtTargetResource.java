...
public class MgmtTargetResource implements MgmtTargetRestApi {
	...
	public ResponseEntity<Void> postAssignedDistributionSet(@PathVariable("controllerId") final String controllerId,
		...
		final UpdateType updateType = (dsId.getUpdateType() != null)
				? MgmtRestModelMapper.convertUpdateType(dsId.getUpdateType()) : UpdateType.COMBINED;

		final Iterator<Target> changed = this.deploymentManagement
				.assignDistributionSet(dsId.getId(), type, dsId.getForcetime(), updateType, controllerId)
	...
	@Override
	public ResponseEntity<Void> postStartUpdate(@PathVariable("controllerId") String controllerId, @PathVariable("actionId") Long actionId,
			boolean force) {
		final Target target = findTargetWithExceptionIfNotFound(controllerId);
		...
		action.setStatus(Status.START);	
		this.deploymentManagement.updateAction(action);
